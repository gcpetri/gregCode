...
int main(){
        ...
        collection_1.insert_item(Stress_ball(Stress_ball_colors::green, Stress_ball_sizes::medium));
        collection_1.make_empty();
        ...
        collection_2.make_empty();
        collection_2.insert_item(Stress_ball(Stress_ball_colors::red, Stress_ball_sizes::large));
        collection_2.remove_this_item(stress_ball_4);
        collection_2.make_empty();
        collection_2.remove_this_item(Stress_ball(Stress_ball_colors::red, Stress_ball_sizes::medium));
        collection_2.remove_any_item();
        ...
        collection_file.insert_item(stress_ball_3);
        sort_by_size(collection_file,Sort_choice::selection_sort);
        sort_by_size(collection_file,Sort_choice::insertion_sort);  
        sort_by_size(collection_file,Sort_choice::bubble_sort);
        cout<<collection_file;
        collection_2.insert_item(stress_ball_3);
        collection_2.insert_item(stress_ball_3);
        collection_2.insert_item(stress_ball_2);
             ...
             if(collection_3.total_items(Stress_ball_colors::yellow) == (collection_file.total_items(Stress_ball_colors::yellow) + collection_2.total_items(Stress_ball_colors::yellow))){
                 if(collection_3.total_items(Stress_ball_sizes::small) == (collection_file.total_items(Stress_ball_sizes::small) + collection_2.total_items(Stress_ball_sizes::small))){
                     ...
                     if((collection_3.total_items() - collection_2.total_items()) == collection_file.total_items()){
                          sort_by_size(collection_4,Sort_choice::selection_sort);
                          sort_by_size(collection_4,Sort_choice::insertion_sort);  
                          sort_by_size(collection_4,Sort_choice::bubble_sort);
                          cout<<collection_4;
                     }
                }
            }
